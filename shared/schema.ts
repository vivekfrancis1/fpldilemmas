...
// League Manager Snapshots - stores actual manager data after each gameweek
export const leagueManagerSnapshots = pgTable("league_manager_snapshots", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  // Additional metrics
  teamValue: decimal("team_value", { precision: 4, scale: 1 }),
  bank: decimal("bank", { precision: 4, scale: 1 }),
  totalTransfers: integer("total_transfers"),
  
  // Source and metadata
  dataSource: varchar("data_source", { length: 50 }).notNull(), // 'user-league', 'creator-league', etc.
  contentCreatorId: integer("content_creator_id").references(() => fplContentCreators.id),
  collectedAt: timestamp("collected_at").defaultNow(),
  isVerified: boolean("is_verified").default(true),
}, (table) => [
  index("idx_league_snapshots_manager_gw").on(table.managerId, table.gameweek),
  index("idx_league_snapshots_rank_gw").on(table.overallRank, table.gameweek),
  index("idx_league_snapshots_league_gw").on(table.leagueId, table.gameweek),
  index("idx_league_snapshots_gameweek").on(table.gameweek),
  uniqueIndex("idx_league_snapshots_unique").on(table.managerId, table.leagueId, table.gameweek),
]);
...
// Ranking benchmarks table - stores actual points needed for specific ranks at each gameweek
export const rankingBenchmarks = pgTable("ranking_benchmarks", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  
  gameweek: integer("gameweek").notNull(),
  season: varchar("season", { length: 10 }).notNull().default("2025/26"),
  rank: integer("rank").notNull(), // 1, 10, 100, 1000, etc.
  pointsRequired: integer("points_required").notNull(),
  
  // Source information
  dataSource: varchar("data_source", { length: 50 }).notNull(), // 'league-data', 'interpolated', etc.
  confidence: decimal("confidence", { precision: 3, scale: 2 }).default("1.0"), // 1.0 = exact data, < 1.0 = interpolated
  
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => [
  index("idx_ranking_benchmarks_gw_rank").on(table.gameweek, table.rank),
  index("idx_ranking_benchmarks_gameweek").on(table.gameweek),
  uniqueIndex("idx_ranking_benchmarks_unique").on(table.gameweek, table.rank),
]);

export type RankingBenchmark = typeof rankingBenchmarks.$inferSelect;
export type InsertRankingBenchmark = typeof rankingBenchmarks.$inferInsert;

export const insertRankingBenchmarkSchema = createInsertSchema(rankingBenchmarks);
