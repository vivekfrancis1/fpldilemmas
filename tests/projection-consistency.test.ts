...
describe('Two-Path Projection Consistency (Aggregator vs Projection-Service)', () => {
  ...
  beforeAll(async () => {
    aggregatorData = await fetchJSON('/api/cached/player-total-points');

    const endGW = Math.min(nextGameweek + 5, 38);
    try {
      projectionServiceData = await fetchJSON(`/api/player-point-breakdowns?startGameweek=${nextGameweek}&endGameweek=${endGW}`);
    } catch {
      projectionServiceData = [];
      console.log('player-point-breakdowns endpoint unavailable — some tests will skip');
    }

    const haalandEl = bootstrapData.elements.find(
      (el: any) => el.second_name === 'Haaland' && el.element_type === 4
    );
    haalandId = haalandEl?.id ?? 0;
  }, 60000);

  it('aggregator endpoint returns players for all 4 positions', () => {
    const positions = new Set(aggregatorData.map((p: any) => p.position));
    const hasGKP = positions.has('GKP') || positions.has('Goalkeeper');
    const hasDEF = positions.has('DEF') || positions.has('Defender');
    const hasMID = positions.has('MID') || positions.has('Midfielder');
    const hasFWD = positions.has('FWD') || positions.has('Forward');
  ...
  it('no player averages more than 8 pts/GW in the aggregator (12-GW total ÷ 12)', () => {
    const gwCount = Object.keys(aggregatorData[0]?.gameweekProjections || {}).length || 12;
    const issues: string[] = [];

    for (const player of aggregatorData) {
      const avgPerGW = (player.totalExpectedPoints || 0) / Math.max(gwCount, 1);
      if (avgPerGW > 8) {
        issues.push(`${player.playerName}: avg=${avgPerGW.toFixed(2)} pts/GW over ${gwCount} GWs`);
      }
    }

    if (issues.length > 0) console.log('Unrealistic aggregator projections:', issues);
    expect(issues.length).toBe(0);
  });

  it('Haaland appears in the top 5 Forwards in the aggregator', () => {
    if (!haalandId) {
      console.log('Skipping: Haaland not found in bootstrap');
      return;
    }

    const forwards = aggregatorData
      .filter((p: any) => p.position === 'FWD' || p.position === 'Forward')
      .sort((a: any, b: any) => b.totalExpectedPoints - a.totalExpectedPoints);

    const haalandRank = forwards.findIndex((p: any) => p.playerId === haalandId);
    console.log(`Aggregator: Haaland rank among FWDs = ${haalandRank + 1} (pts: ${forwards[haalandRank]?.totalExpectedPoints?.toFixed(2)})`);
    console.log('Top 5 FWDs (aggregator):', forwards.slice(0, 5).map((p: any) => `${p.playerName}=${p.totalExpectedPoints?.toFixed(2)}`));

    expect(haalandRank).toBeLessThan(5);
  });

  it('projection-service endpoint returns data', () => {
    if (projectionServiceData.length === 0) {
      console.log('Skipping: projection-service data unavailable');
      return;
    }
    expect(projectionServiceData.length).toBeGreaterThan(100);
  });

  it('no player has unrealistic per-GW average in projection-service (6-GW window, max 8 pts/GW)', () => {
    if (projectionServiceData.length === 0) {
      console.log('Skipping: projection-service data unavailable');
      return;
    }
    ...
    for (const player of projectionServiceData) {
      const gwKeys = Object.keys(player.gameweekProjections || {});
      const gwCount = Math.max(gwKeys.length, 1);
      const totalPts = gwKeys.reduce((sum: number, gw: string) => sum + (player.gameweekProjections[gw] || 0), 0);
      const avgPerGW = totalPts / gwCount;

      if (avgPerGW > 8) {
        issues.push(`${player.name || player.playerName}: avg=${avgPerGW.toFixed(2)} pts/GW`);
      }
    }

    if (issues.length > 0) console.log('Unrealistic projection-service projections:', issues);
    expect(issues.length).toBe(0);
  });

  it('Haaland appears in the top 5 Forwards in the projection-service', () => {
    if (projectionServiceData.length === 0 || !haalandId) {
      console.log('Skipping: data unavailable');
      return;
    }

    const forwards = projectionServiceData
    ...
    const haalandRank = forwards.findIndex((p: any) => p.playerId === haalandId);
    if (haalandRank < 0) {
      console.log('Haaland not found in projection-service FWD list');
    } else {
      const haalandTotal = Object.values(forwards[haalandRank].gameweekProjections || {})
        .reduce((s: number, v) => s + (v as number), 0);
      console.log(`Projection-service: Haaland rank among FWDs = ${haalandRank + 1} (total: ${haalandTotal.toFixed(2)})`);
      console.log('Top 5 FWDs (projection-service):', forwards.slice(0, 5).map((p: any) => {
        const t = Object.values(p.gameweekProjections || {}).reduce((s: number, v) => s + (v as number), 0);
        return `${p.name || p.playerName}=${t.toFixed(2)}`;
      }));
    }
  ...
  it('top FWD in aggregator is in the top 3 FWDs of the projection-service', () => {
    ...
    console.log('Top 3 FWDs (projection-service):', psFwds.slice(0, 3).map((p: any) => p.name || p.playerName));

    expect(rankInPS).toBeGreaterThanOrEqual(0);
    expect(rankInPS).toBeLessThan(3);
  });
  ...
  it('projection-service goal points for top FWD exceed those of any individual DEF (FWD premium)', () => {
    ...
    const getTotal = (p: any) =>
      Object.values(p.gameweekProjections || {}).reduce((s: number, v) => s + (v as number), 0);

    const topFwd = projectionServiceData
      .filter((p: any) => p.position === 'FWD')
      .sort((a: any, b: any) => getTotal(b) - getTotal(a))[0];

    const topDef = projectionServiceData
      .filter((p: any) => p.position === 'DEF')
      .sort((a: any, b: any) => {
        const aGoals = Object.values(a.pointsFromGoals || {}).reduce((s: number, v) => s + (v as number), 0);
        const bGoals = Object.values(b.pointsFromGoals || {}).reduce((s: number, v) => s + (v as number), 0);
        return bGoals - aGoals;
      })[0];

    if (!topFwd || !topDef) return;

    const fwdGoalPts = Object.values(topFwd.pointsFromGoals || {}).reduce((s: number, v) => s + (v as number), 0);
    const defGoalPts = Object.values(topDef.pointsFromGoals || {}).reduce((s: number, v) => s + (v as number), 0);

    console.log(`Top FWD goal pts (${topFwd.name || topFwd.playerName}): ${fwdGoalPts.toFixed(2)}`);
    console.log(`Top DEF goal pts (${topDef.name || topDef.playerName}): ${defGoalPts.toFixed(2)}`);

    expect(fwdGoalPts).toBeGreaterThan(defGoalPts);
  });
});
