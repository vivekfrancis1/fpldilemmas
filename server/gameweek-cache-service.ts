  ...
  async getCachedPlayerData(playerIds: number[], gameweek: number, season: string = this.CURRENT_SEASON): Promise<GameweekPlayerData[]> {
    try {
      ...
      return result.rows.map(row => ({
        ...
        expected_goals: (row as any).expected_goals || 0,
        expected_assists: (row as any).expected_assists || 0,
        expected_goal_involvements: (row as any).expected_goal_involvements || 0,
        expected_goals_conceded: (row as any).expected_goals_conceded || 0,
        influence: (row as any).influence || 0,
        creativity: (row as any).creativity || 0,
        threat: (row as any).threat || 0,
        ict_index: (row as any).ict_index || 0,
        value: (row as any).value || 0,
        selected: (row as any).selected || 0,
        transfers_in: (row as any).transfers_in || 0,
        transfers_out: (row as any).transfers_out || 0,
        team_h_score: (row as any).team_h_score ?? null,
        team_a_score: (row as any).team_a_score ?? null,
