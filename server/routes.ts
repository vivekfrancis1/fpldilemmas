...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get sell price overrides for a manager
  app.get("/api/manager/:managerId/sell-price-overrides", async (req, res) => {
    try {
      const { managerId } = req.params;
      ...
      console.log(`💰 Fetching sell price overrides for manager ${managerId}`);
      
      const result = await db.execute(sql`
        SELECT player_id, sell_price 
        FROM sell_price_overrides 
        WHERE manager_id = ${parseInt(managerId)}
      `);
      
      const overrides: Record<number, number> = {};
      for (const row of result.rows as any[]) {
        overrides[row.player_id] = row.sell_price;
      }
      
      console.log(`✅ Retrieved ${Object.keys(overrides).length} sell price override(s)`);
      res.json({ overrides });
      
    } catch (error) {
      console.error(`❌ Error fetching sell price overrides for manager ${req.params.managerId}:`, error);
      res.status(500).json({
        error: "Failed to fetch sell price overrides",
        message: error instanceof Error ? error.message : "Unknown error",
      });
    }
  });

  // Save or update sell price override for a player
  app.post("/api/manager/:managerId/sell-price-overrides", async (req, res) => {
    try {
      const { managerId } = req.params;
      const { playerId, sellPrice } = req.body;
      ...
      console.log(`💰 Saving sell price override for manager ${managerId}, player ${playerId}: ${sellPrice}`);
      
      // Upsert sell price override
      await db.execute(sql`
        INSERT INTO sell_price_overrides (manager_id, player_id, sell_price, updated_at)
        VALUES (${parseInt(managerId)}, ${parseInt(playerId)}, ${parseInt(sellPrice)}, NOW())
        ON CONFLICT (manager_id, player_id) 
        DO UPDATE SET sell_price = ${parseInt(sellPrice)}, updated_at = NOW()
      `);
      
      console.log(`✅ Sell price override saved successfully`);
      res.json({ success: true });
      
    } catch (error) {
      console.error(`❌ Error saving sell price override:`, error);
      res.status(500).json({
        error: "Failed to save sell price override",
        message: error instanceof Error ? error.message : "Unknown error",
      });
    }
  });

  // Delete sell price override for a player
  app.delete("/api/manager/:managerId/sell-price-overrides/:playerId", async (req, res) => {
    try {
      const { managerId, playerId } = req.params;
      ...
      console.log(`💰 Deleting sell price override for manager ${managerId}, player ${playerId}`);
      
      await db.execute(sql`
        DELETE FROM sell_price_overrides 
        WHERE manager_id = ${parseInt(managerId)} AND player_id = ${parseInt(playerId)}
      `);
      
      console.log(`✅ Sell price override deleted successfully`);
      res.json({ success: true });
      
    } catch (error) {
      console.error(`❌ Error deleting sell price override:`, error);
      res.status(500).json({
        error: "Failed to delete sell price override",
        message: error instanceof Error ? error.message : "Unknown error",
      });
    }
  });
