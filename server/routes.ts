...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ============================================================================
  // TRANSFER PLANNER API ENDPOINTS
  // ============================================================================
  ...
  app.get("/api/transfer-planner/drafts/:managerId", async (req, res) => {
    try {
      ...
      const drafts = await db.select()
        ...
        .orderBy(desc(transferPlannerDrafts.updatedAt));

      // Fetch players for each draft
      const draftsWithPlayers = await Promise.all(
      ...
      res.json({ success: true, drafts: draftsWithPlayers });
    } catch (error) {
  ...
  app.get("/api/transfer-planner/draft/:draftId", async (req, res) => {
    try {
      const { draftId } = req.params;

      const drafts = await db.select()
        .from(transferPlannerDrafts)
        .where(eq(transferPlannerDrafts.id, draftId))
        .limit(1);
      ...
      const players = await db.select()
        .from(draftPlayers)
        .where(eq(draftPlayers.draftId, draftId))
        .orderBy(asc(draftPlayers.squadPosition));
  ...
  app.post("/api/transfer-planner/drafts", async (req, res) => {
    try {
      const { managerId, gameweek, draftName, mode, formation, players } = req.body;

      if (!managerId || !gameweek || !draftName || !mode) {
        return res.status(400).json({ 
          error: "Missing required fields: managerId, gameweek, draftName, mode" 
        });
      }

      // Calculate total projected points
      const totalProjectedPoints = players?.reduce((sum: number, p: any) => 
        sum + (parseFloat(p.projectedPoints) || 0), 0
      ) || 0;

      // Calculate team value
      const teamValue = players?.reduce((sum: number, p: any) => 
        sum + (parseFloat(p.price) || 0), 0
      ) || 0;

      // Create draft
      const [draft] = await db.insert(transferPlannerDrafts).values({
        managerId,
        gameweek,
        draftName,
        mode,
        ...
        transfersUsed: players?.filter((p: any) => p.isTransferIn).length || 0,
        teamValue: teamValue.toString()
      }).returning();

      // Insert players if provided
      if (players && players.length > 0) {
        await db.insert(draftPlayers).values(
          players.map((p: any) => ({
            draftId: draft.id,
            playerId: p.playerId,
            ...
            isTransferIn: p.isTransferIn || false,
            isTransferOut: p.isTransferOut || false,
            replacedPlayerId: p.replacedPlayerId || null,
            projectedPoints: (p.projectedPoints || 0).toString()
          }))
        );
      }
  ...
  // Update a draft
  app.patch("/api/transfer-planner/draft/:draftId", async (req, res) => {
    try {
      const { draftId } = req.params;
      const { draftName, formation, players } = req.body;

      // Calculate totals if players provided
      let totalProjectedPoints, teamValue, transfersUsed;
      if (players) {
        totalProjectedPoints = players.reduce((sum: number, p: any) => 
          sum + (parseFloat(p.projectedPoints) || 0), 0
        );
        teamValue = players.reduce((sum: number, p: any) => 
          sum + (parseFloat(p.price) || 0), 0
        );
        transfersUsed = players.filter((p: any) => p.isTransferIn).length;
      }
      ...
      if (draftName) updateData.draftName = draftName;
      if (formation) updateData.formation = formation;
      if (totalProjectedPoints !== undefined) updateData.totalProjectedPoints = totalProjectedPoints.toString();
      if (teamValue !== undefined) updateData.teamValue = teamValue.toString();
      if (transfersUsed !== undefined) updateData.transfersUsed = transfersUsed;

      await db.update(transferPlannerDrafts)
        .set(updateData)
        .where(eq(transferPlannerDrafts.id, draftId));
      ...
      if (players) {
        ...
        await db.delete(draftPlayers).where(eq(draftPlayers.draftId, draftId));
      ...
      // Fetch updated draft
      const updatedDrafts = await db.select()
        .from(transferPlannerDrafts)
        .where(eq(transferPlannerDrafts.id, draftId))
        .limit(1);
      ...
      res.json({ success: true, draft: { ...updatedDraft, players: updatedPlayers } });
    } catch (error) {
  ...
  // Delete a draft
  app.delete("/api/transfer-planner/draft/:draftId", async (req, res) => {
    try {
      const { draftId } = req.params;

      await db.delete(transferPlannerDrafts).where(eq(transferPlannerDrafts.id, draftId));
  ...
  console.log("✓ Transfer Planner API routes registered successfully");
