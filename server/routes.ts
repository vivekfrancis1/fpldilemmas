...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Player Assists Configuration Admin API
  app.get('/api/admin/assist-config', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      const config = await assistConfigService.getAllConfig();
      res.json(config);
    } catch (error) {
      console.error('Error getting assist configuration:', error);
      res.status(500).json({ message: 'Failed to get assist configuration' });
    }
  });

  app.post('/api/admin/assist-config/elite-boost', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.addEliteBoost(req.body);
      res.json({ success: true });
    } catch (error) {
      console.error('Error adding elite boost:', error);
      res.status(500).json({ message: 'Failed to add elite boost' });
    }
  });

  app.put('/api/admin/assist-config/elite-boost/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.updateEliteBoost(parseInt(req.params.id), req.body);
      res.json({ success: true });
    } catch (error) {
      console.error('Error updating elite boost:', error);
      res.status(500).json({ message: 'Failed to update elite boost' });
    }
  });

  app.delete('/api/admin/assist-config/elite-boost/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.deleteEliteBoost(parseInt(req.params.id));
      res.json({ success: true });
    } catch (error) {
      console.error('Error deleting elite boost:', error);
      res.status(500).json({ message: 'Failed to delete elite boost' });
    }
  });

  app.put('/api/admin/assist-config/historical-tier/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.updateHistoricalTier(parseInt(req.params.id), req.body);
      res.json({ success: true });
    } catch (error) {
      console.error('Error updating historical tier:', error);
      res.status(500).json({ message: 'Failed to update historical tier' });
    }
  });

  app.put('/api/admin/assist-config/creativity-tier/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.updateCreativityTier(parseInt(req.params.id), req.body);
      res.json({ success: true });
    } catch (error) {
      console.error('Error updating creativity tier:', error);
      res.status(500).json({ message: 'Failed to update creativity tier' });
    }
  });

  app.put('/api/admin/assist-config/price-tier/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.updatePriceTier(parseInt(req.params.id), req.body);
      res.json({ success: true });
    } catch (error) {
      console.error('Error updating price tier:', error);
      res.status(500).json({ message: 'Failed to update price tier' });
    }
  });

  app.put('/api/admin/assist-config/position-rate/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.updatePositionRate(parseInt(req.params.id), req.body);
      res.json({ success: true });
    } catch (error) {
      console.error('Error updating position rate:', error);
      res.status(500).json({ message: 'Failed to update position rate' });
    }
  });

  app.put('/api/admin/assist-config/general/:id', async (req, res) => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.updateGeneralConfig(parseInt(req.params.id), req.body);
      res.json({ success: true });
    } catch (error) {
  ...
  // Initialize assist configuration on startup
  (async () => {
    try {
      const { assistConfigService } = await import('./assist-config-service');
      await assistConfigService.initializeDefaults();
    } catch (error) {
      console.error('Error initializing assist configuration:', error);
    }
  })();
