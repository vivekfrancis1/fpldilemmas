...
class ProjectionCacheWorker {
  ...
  private async cacheGoalsProjections(): Promise<void> {
    ...
    try {
      ...
      // PROJECTIONS INHERENTLY BALANCED: Using goal share × team goals methodology
      console.log(`✅ Goal projections calculated using goal share × team goals - inherently balanced, no normalization needed`);
  ...
  private async cacheAssistProjections(): Promise<void> {
    try {
      ...
      // PROJECTIONS INHERENTLY BALANCED: Using assist share × team assists methodology
      console.log(`✅ Assist projections calculated using assist share × team assists - inherently balanced, no normalization needed`);
  ...
  async getCacheStats(): Promise<any> {
    try {
      ...
      const [goals, assists, cleanSheets, minutes, defensive, teams, goalShare, assistShare] = await Promise.all([
        db.select({ 
          count: sql`count(*)`,
  ...
  private async getPlayerTotalPointsStatus(): Promise<{ type: string; count: number; lastUpdated: string | null; isStale: boolean }> {
    try {
      ...
      const { playerTotalPointsSnapshots } = await import("@shared/schema");
      const { sql, count } = await import("drizzle-orm");
      
      const now = new Date();
      const STALE_THRESHOLD = 60 * 60 * 1000; // 1 hour (player total points are more complex, allow longer cache)
      
      // Get count and most recent update time from playerTotalPointsSnapshots
      const result = await db
        .select({
          count: count(),
          lastUpdated: sql<string>`MAX(${playerTotalPointsSnapshots.createdAt})::text`
        })
