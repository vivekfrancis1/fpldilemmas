...
- **Live Goal Tweets**: Automated live goal monitoring via `LiveGoalMonitor` service that polls FPL fixtures API every 60 seconds during live matches. Detects new goals by comparing player stats snapshots between polls using the FPL `explain` data per fixture. Only tweets when the goal scorer or assist provider has >5% ownership. Tweet includes scorer name, assister (when safely attributable), ownership %, updated score, minute, and link to match stats page. Groups fixtures by gameweek for correct API calls. Also tweets yellow cards, red cards, save points (3+ saves threshold), and defensive contribution points (DEF/GKP ≥10, MID/FWD ≥12) for players with >5% ownership. Threshold-based events (saves, DC) use deduplication to prevent repeat tweets. Bonus points tweeted when match finishes with 12-hour post-match monitoring for BPS recalculations — sends update tweet if bonus allocation changes. Admin endpoints: `/api/admin/live-goals/status` and `/api/admin/live-goals/preview`.
