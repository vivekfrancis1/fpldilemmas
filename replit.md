...
FPL Content Creators Admin: Streamlined admin interface with only essential fields: Name, Manager ID, Description, Twitter Handle, and YouTube URL. Removed Handle, Platform, and Website fields per user request. Both Twitter and YouTube fields are optional and can be left blank. Database schema updated accordingly with proper column removal. All content creators now have complete Twitter handles and YouTube URLs populated with authentic social media data. Interface displays full URLs as clickable links and Twitter handles as styled text. Complete CRUD functionality implemented including delete feature with confirmation dialog and red-styled delete button next to edit button. Admin URL updated from /admin to /admin-content-creators for better specificity. **PRODUCTION ADMIN ACCESS IMPLEMENTED (Aug 28, 2025):** All admin tools (admin-content-creators, admin-goal-projections, admin-upset-config) now available in production with secure authentication. Access requires ADMIN_SECRET_KEY parameter in URL (?admin_key=SECRET). Development environment allows access without key. Frontend automatically handles admin key authentication for all /api/admin endpoints.

Development-Only Tools: The following tools are restricted to development environment only and not accessible in production: /transfers, /captain, /watchlist, /price-tracker, /league-comparison, /results-projections, /team-assist-projections, /goal-share, /assist-share, /player-goal-projections, /player-assist-projections, /player-goals-scored-projections, /season-projections. These tools appear in the sidebar under "Under Development" section in development environment only and are in active development phase.
