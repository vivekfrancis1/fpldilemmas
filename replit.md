...
Most popular tools: Player Statistics, My Live Rank, and Fixture Analyzer (updated January 2025)
Fixture Analyzer display: Shows 6 gameweeks by default (GW3-GW8) with user customization up to GW38 for comprehensive fixture analysis (August 2025)
...
Goal Range Compression: Successfully implemented realistic Premier League goal distributions by compressing team season totals from unrealistic 23-95 goals to professional 30-85 goal range. Applied comprehensive variance compression (96-104% → 99-101%), tightened market bounds (0.3-4.2 → 0.6-2.2 goals per match), compressed attacking tier multipliers, and adjusted defensive multipliers (0.45-1.75 → 0.65-1.40). Achieved final ranges: Goals Scored 34-88 goals, Goals Against 29-70 goals, maintaining perfect mathematical consistency where every goal scored equals one goal conceded (August 2025)
Clean Sheet Formula: Updated to gameweek-specific calculation using CS% = 55 - 15 × Goals Against for that particular gameweek. Formula creates realistic clean sheet percentages that properly scale with defensive performance: strong defensive gameweeks (0.5 GA) yield ~47.5% CS chance, average gameweeks (1.5 GA) yield ~32.5% CS chance, poor defensive gameweeks (3.0 GA) yield ~10% CS chance. Ensures logical relationship where fewer goals against equals higher clean sheet probability (August 2025)
...
- **Daily Price Tracking**: Automated system fetching player prices, ownership, and transfer data at 7:30 AM IST daily, calculating daily transfer differences and storing historical trends for comprehensive price analysis. Recent Price Changes shows all season price changes sorted by recency (most recent first), displaying start price → current price with total season change and recent gameweek changes. Price Predictions shows players likely to rise/fall based on transfer activity, optimized for fast loading (~200ms) and correctly displays predicted prices respecting FPL's 0.1m daily/0.3m gameweek limits. Fixed display issues showing proper price formatting and "Stable" indicators for unchanged predictions (August 2025)
