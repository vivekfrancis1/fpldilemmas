            ...
            return gks.length > 0 && (
              <div className="flex justify-center gap-1 sm:gap-0.5">
                {gks.map(player => {
                  ...
                  return (
                    <div key={player.element} className="flex flex-col items-center w-[24%] sm:w-[19%]" data-testid={`pitch-player-${player.element}`}>
                      <div className="relative w-full">
                        <svg viewBox="0 0 403 302" className="w-full drop-shadow-md sm:drop-shadow-lg md:drop-shadow-xl">
                          ...
                          <text x="202" y="112" className="text-[44px] sm:text-[32px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.team_short_name || 'UNK'}</text>
                          <text x="202" y="165" className="text-[48px] sm:text-[34px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.web_name || player.player_name || 'Unknown'}</text>
                          <text x="202" y="218" className="text-[54px] sm:text-[38px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{(player.event_points || 0) * (player.is_captain ? 2 : player.multiplier || 1)}</text>
                          {showFixtures && getNextFixtures && player.team_id && (() => {
                            const fixtures = getNextFixtures(player.team_id, 1);
                            if (fixtures.length > 0) {
                              const fixture = fixtures[0];
                              return <text x="202" y="268" className="text-[40px] sm:text-[30px]" fontWeight="bold" textAnchor="middle" fill={textColor}>vs {fixture.opponent} {fixture.isHome ? '(H)' : '(A)'}</text>;
                            }
                            return null;
                          })()}
                        </svg>
            ...
            <div className="mt-6 sm:mt-8 md:mt-10 pt-4 sm:pt-6 border-t-2 border-white/30">
              ...
              <div className="flex justify-center gap-1 sm:gap-0.5">
                {sortedBench.map(player => {
                  ...
                  return (
                    <div key={player.element} className="flex flex-col items-center w-[24%] sm:w-[19%] opacity-90" data-testid={`bench-player-${player.element}`}>
                      <div className="relative w-full">
                        <svg viewBox="0 0 403 302" className="w-full drop-shadow-md sm:drop-shadow-lg">
                          ...
                          <text x="202" y="165" className="text-[44px] sm:text-[34px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.web_name || player.player_name || 'Unknown'}</text>
