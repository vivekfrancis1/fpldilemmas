      ...
      <div className="relative bg-gradient-to-b from-green-600 to-green-700 rounded-none sm:rounded-lg p-4 sm:p-6 md:p-8 lg:p-10 overflow-hidden h-full flex flex-col justify-center">
        ...
        <div className="relative space-y-10 sm:space-y-6 md:space-y-8 lg:space-y-10">
          ...
          {(() => {
            ...
            return gks.length > 0 && (
              <div className="flex justify-center gap-1 sm:gap-0.5">
                {gks.map(player => {
                  const jerseyColor = getTeamJerseyColor(player.team_id || 0);
                  const textColor = getTextColor(jerseyColor);
                  
                  return (
                    <div key={player.element} className="flex flex-col items-center w-[33%] sm:w-[18%]" data-testid={`pitch-player-${player.element}`}>
                      <div className="relative w-full">
                        <svg viewBox="0 0 403 302" className="w-full drop-shadow-md sm:drop-shadow-lg md:drop-shadow-xl">
                          ...
                          <text x="202" y="112" className="text-[63px] sm:text-[34px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.team_short_name || 'UNK'}</text>
                          <text x="202" y="165" className="text-[66px] sm:text-[36px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.web_name || player.player_name || 'Unknown'}</text>
                          <text x="202" y="218" className="text-[74px] sm:text-[40px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{(player.event_points || 0) * (player.is_captain ? 2 : player.multiplier || 1)}</text>
                          {showFixtures && getNextFixtures && player.team_id && (() => {
                            const fixtures = getNextFixtures(player.team_id, 1);
                            if (fixtures.length > 0) {
                              const fixture = fixtures[0];
                              return <text x="202" y="268" className="text-[59px] sm:text-[32px]" fontWeight="bold" textAnchor="middle" fill={textColor}>vs {fixture.opponent} {fixture.isHome ? '(H)' : '(A)'}</text>;
                            }
                            return null;
                          })()}
                        </svg>
            ...
            <div className="mt-6 sm:mt-8 md:mt-10 pt-4 sm:pt-6 border-t-2 border-white/30">
              ...
              <div className="flex justify-center gap-1 sm:gap-0.5">
                {sortedBench.map(player => {
                  ...
                  return (
                    <div key={player.element} className="flex flex-col items-center w-[33%] sm:w-[18%] opacity-90" data-testid={`bench-player-${player.element}`}>
                      <div className="relative w-full">
                        <svg viewBox="0 0 403 302" className="w-full drop-shadow-md sm:drop-shadow-lg">
                          ...
                          <path d="M 130 14 L 144 26 L 158 36 L 173 42 Q 187 42 202 42 L 216 42 Q 230 42 245 36 L 259 26 L 274 14" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1.5" />
                          {player.in_dreamteam && (<g><circle cx="307" cy="72" r="17" fill="#A855F7" stroke="white" strokeWidth="2.5" /><path d="M 307 63 L 309 69 L 315 69 L 310 73 L 312 79 L 307 75 L 302 79 L 304 73 L 299 69 L 305 69 Z" fill="white" /></g>)}
                          <text x="202" y="112" className="text-[65px] sm:text-[34px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.team_short_name || 'UNK'}</text>
                          <text x="202" y="165" className="text-[71px] sm:text-[36px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.web_name || player.player_name || 'Unknown'}</text>
                          <text x="202" y="218" className="text-[78px] sm:text-[40px]" fontWeight="bold" textAnchor="middle" fill={textColor}>{player.event_points || 0}</text>
                          {showFixtures && getNextFixtures && player.team_id && (() => {
                            const fixtures = getNextFixtures(player.team_id, 1);
                            if (fixtures.length > 0) {
                              const fixture = fixtures[0];
                              return <text x="202" y="268" className="text-[62px] sm:text-[32px]" fontWeight="bold" textAnchor="middle" fill={textColor}>vs {fixture.opponent} {fixture.isHome ? '(H)' : '(A)'}</text>;
                            }
                            return null;
                          })()}
                        </svg>
