                ...
                <div className="flex flex-wrap gap-0.5 sm:gap-1">
                  ...
                  {teams.map(team => (
                    <button key={team}
                      ...
                      className={`chip-toggle rounded-full border text-[10px] sm:text-xs font-medium px-1.5 sm:px-2.5 py-px sm:py-0.5 leading-none cursor-pointer transition-colors ${selectedTeam === team ? 'bg-indigo-100 text-indigo-700 border-indigo-300' : 'bg-gray-100 text-gray-500 border-gray-300'}`}>
                      {team}
                    </button>
                  ))}
                </div>
                      ...
                      >
                        <td className="px-1 md:px-3 py-2 md:py-3 sticky left-0 bg-white border-r border-gray-200 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] z-20 w-[130px] min-w-[130px]">
                          <div className="flex items-center gap-1">
                            <JerseyIcon team={player.teamShort} position={player.position} className="h-7 w-7 object-contain shrink-0" />
