...
export default function TeamAssistProjections() {
  ...
  const { defaultStart, defaultEnd } = useMemo(() => {
    ...
    const effectiveStart = (currentGWHasUnstarted && currentGameweek > 0 && range.startGameweek && currentGameweek < parseInt(range.startGameweek))
      ...
      : range.startGameweek;
    // Default the end to the last gameweek with real Odds API coverage.
    const effectiveEnd = (hasTBCFixture && fixtureMode === 'base')
      ? "39"
      : (maxGameweekWithOdds !== null && effectiveStart && maxGameweekWithOdds >= parseInt(effectiveStart))
        ? String(maxGameweekWithOdds)
        : (range.endGameweek || "13");
    return {
  ...
  // Same total/average math as the displayed Total and Avg/Game cells (excluding a decided
  ...
  const filteredProjections = useMemo(() => {
    ...
    return resolvedProjections
      ...
      .sort((a, b) => {
        ...
        switch (sortBy) {
          case "total": return getRowStats(b).total - getRowStats(a).total;
          case "average": return getRowStats(b).avg - getRowStats(a).avg;
          case "position": return a.position - b.position;
          default: return getRowStats(b).avg - getRowStats(a).avg;
        }
      });
  ...
  const gwSourceMap = useMemo(() => {
    ...
    for (const team of resolvedProjections) {
      for (const [gwStr, fixtures] of Object.entries((team as any).fixtureDetails || {})) {
        const gwNumber = parseInt(gwStr);
        const hasOdds = ((fixtures as any[]) || []).some((f: any) => f.source === 'odds');
        if (hasOdds) {
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3 items-end">
              <div className="flex items-center gap-1.5">
                ...
                <Select value={startGameweek} onValueChange={setStartGameweek}>
                  <SelectTrigger className={`h-7 text-xs ${hasTBCFixture && fixtureMode === 'base' ? 'w-32' : 'w-20'}`} data-testid="select-start-gameweek">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <Select value={endGameweek} onValueChange={setEndGameweek}>
                  <SelectTrigger className={`h-7 text-xs ${hasTBCFixture && fixtureMode === 'base' ? 'w-32' : 'w-20'}`} data-testid="select-end-gameweek">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {availableGameweeks.map((gw) => (
