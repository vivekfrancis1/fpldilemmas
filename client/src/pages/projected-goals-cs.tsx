      ...
      <div className="fpl-page-header">
        <div className="fpl-page-header-content">
          <div className="fpl-page-title">
            ...
            <h1>{viewMode === "future" ? "Match Predictions" : "Match Results"}</h1><SeasonBadge />
          </div>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex flex-wrap gap-0.5 sm:gap-1">
                      <button onClick={() => setSelectedTeam("all")}
                        className={`chip-toggle rounded-full border text-xs font-medium px-2.5 sm:px-3 py-1 leading-none cursor-pointer transition-colors ${selectedTeam === "all" ? 'bg-indigo-100 text-indigo-700 border-indigo-300' : 'bg-gray-100 text-gray-500 border-gray-300'}`}>
                        All
                      </button>
                      {bootstrapData?.teams?.sort((a, b) => a.short_name.localeCompare(b.short_name)).map(team => (
                        <button key={team.id}
                          ...
                          className={`chip-toggle rounded-full border text-xs font-medium px-2.5 sm:px-3 py-1 leading-none cursor-pointer transition-colors ${selectedTeam === team.short_name ? 'bg-indigo-100 text-indigo-700 border-indigo-300' : 'bg-gray-100 text-gray-500 border-gray-300'}`}>
                          {team.short_name}
                        </button>
                      ))}
                    </div>
            ...
            <CardContent className="p-0">
              <div className="space-y-0">
                {Object.entries(groupedProjections).map(([groupKey, projections]) => {
                  ...
                  return (
                    <div key={groupKey}>
                      {/* Day Header - Compact. On mobile this line also carries the GOALS/CS%/RESULT
                      ...
                      <div className="bg-gradient-to-r from-gray-100 to-gray-50 px-3 py-1.5 border-b border-gray-200">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="hidden lg:inline text-xs text-gray-600 font-medium">{projections.length} {projections.length === 1 ? 'match' : 'matches'}</span>
                        </div>
                      </div>

                      {/* Column Headers - Desktop only: single or dual columns based on match count */}
                          ...
                          return (
                            <div key={`pair-${pairIndex}`} className="grid grid-cols-1 lg:grid-cols-2 gap-1.5">
                              ...
                              {match1 && (() => {
                                ...
                                return (
                                <div
                                  className={`relative bg-white rounded-lg border border-gray-200 shadow-md hover:shadow-lg transition-all duration-200 overflow-hidden ${oddsEventId1 ? 'cursor-pointer' : ''}`}
                              ...
                              {/* Second Match */}
                              {match2 && (() => {
                                const oddsEventId2 = oddsHistoryEventIdByTeams.get(`${match2.homeTeam.id}-${match2.awayTeam.id}`);
                                return (
                                <div
                                  className={`relative bg-white rounded-lg border border-gray-200 shadow-md hover:shadow-lg transition-all duration-200 overflow-hidden ${oddsEventId2 ? 'cursor-pointer' : ''}`}
