...
const projPtsToAssists = (pts: number): number => {
  if (pts === 0) return 0;
  return pts / 3;
};
...
const projPtsToSaves = (pts: number): number => {
  if (pts === 0) return 0;
  return pts * 3;
};
...
export default function ProjectionAccuracy() {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'error':
          ...
          break;
        case 'proj_goals':
          comparison = projPtsToGoals(parseFloat(a.projected_goals || '0'), a.position) - projPtsToGoals(parseFloat(b.projected_goals || '0'), b.position);
          break;
        case 'act_goals':
          comparison = (a.actual_goals || 0) - (b.actual_goals || 0);
          break;
        case 'proj_assists':
          comparison = projPtsToAssists(parseFloat(a.projected_assists || '0')) - projPtsToAssists(parseFloat(b.projected_assists || '0'));
          break;
        case 'act_assists':
          comparison = (a.actual_assists || 0) - (b.actual_assists || 0);
          break;
        case 'proj_cs':
          comparison = projPtsToCS(parseFloat(a.projected_clean_sheet || '0'), a.position) - projPtsToCS(parseFloat(b.projected_clean_sheet || '0'), b.position);
          break;
        case 'act_cs':
          comparison = (a.actual_clean_sheet || 0) - (b.actual_clean_sheet || 0);
          break;
        case 'proj_bonus':
          comparison = parseFloat(a.projected_bonus || '0') - parseFloat(b.projected_bonus || '0');
          break;
        case 'act_bonus':
          comparison = (a.actual_bonus || 0) - (b.actual_bonus || 0);
          break;
        case 'proj_saves':
          comparison = projPtsToSaves(parseFloat(a.projected_saves || '0')) - projPtsToSaves(parseFloat(b.projected_saves || '0'));
          break;
        case 'act_saves':
          comparison = (a.actual_saves || 0) - (b.actual_saves || 0);
          break;
      }
  ...
  const playerStats = useMemo(() => {
    ...
    const totalProjCS = compareSet.reduce((sum, p) => sum + projPtsToCS(parseFloat(p.projected_clean_sheet || '0'), p.position), 0);
