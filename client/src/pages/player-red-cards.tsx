...
export default function PlayerRedCards() {
  ...
  const allGameweeks = redCardProjections && redCardProjections.length > 0 
  ...
  const filteredProjections = resolvedProjections.filter((projection: RedCardProjection) => {
    ...
    let aValue: number;
    let bValue: number;
    if (sortBy === "totalRedCards") {
      aValue = displayGWs.reduce((sum, gw) => sum + (a.redCards[`gw${gw}`] || 0), 0);
      bValue = displayGWs.reduce((sum, gw) => sum + (b.redCards[`gw${gw}`] || 0), 0);
    } else {
      aValue = displayGWs.reduce((sum, gw) => sum + (a.pointsFromRedCards[`gw${gw}`] || 0), 0);
      bValue = displayGWs.reduce((sum, gw) => sum + (b.pointsFromRedCards[`gw${gw}`] || 0), 0);
    }
      ...
      <div className="fpl-page-header">
        <div className="fpl-page-header-content">
          <div className="fpl-page-title">
            ...
            <h1>Player Red Cards</h1><SeasonBadge />
          </div>
                  ...
                  <table className="fpl-table">
                    <thead>
                      <tr>
                        ...
                        <th className="text-center cursor-pointer fpl-col-stat" onClick={() => handleSort("totalRedCards")}>
                    ...
                    <tbody>
                      {filteredProjections.map((projection: RedCardProjection) => {
                        const displayTotalRC = displayGWs.reduce((sum, gw) => sum + (projection.redCards[`gw${gw}`] || 0), 0);
                        return (
              ...
              <div className="fpl-card-content p-0">
                <div className="overflow-x-auto">
                  <table className="fpl-table">
                    ...
                    <tbody>
                      {filteredProjections.map((projection: RedCardProjection) => {
                        const displayTotalPts = displayGWs.reduce((sum, gw) => sum + (projection.pointsFromRedCards[`gw${gw}`] || 0), 0);
                        return (
                        <tr key={projection.playerId}>
