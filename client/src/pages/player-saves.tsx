...
import { Shield, Search, ArrowUpDown, Users, X, Filter, ChevronDown, ChevronUp, ChevronRight, History, Calendar } from "lucide-react";
...
export default function PlayerSaves() {
  ...
  // On mobile, size gameweek/Saves columns so a whole number always fits the visible width at
      ...
      <div className="fpl-page-container">
        <div className="fpl-page-header">
          <div className="fpl-page-header-content">
            <div className="fpl-page-title">
              <Shield className="h-8 w-8" />
              <h1>{viewMode === "future" ? "Goalkeeper Saves Projections" : "Goalkeeper Saves History"}</h1><SeasonBadge season={viewMode === "past" ? (historySeason ?? PREVIOUS_SEASON) : undefined} />
            </div>
            <p className="fpl-page-subtitle">
              {viewMode === "future"
        ...
        <div className="fpl-card mb-2">
          <div className="p-2 sm:p-3">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
              ...
              <div className="flex items-center gap-2 text-xs text-gray-600 justify-center sm:justify-end col-span-2 sm:col-span-1">
                  ...
                  <table className="min-w-full divide-y divide-gray-200 [&_th]:snap-start [&_td]:snap-start">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        ...
                        <th
                          className={`px-1 md:px-3 py-2 text-center text-xs md:text-sm font-medium text-gray-500 uppercase tracking-wider border-l border-gray-200 bg-blue-50 md:sticky md:right-[65px] z-[5] ${!columnWidth ? 'w-[65px] min-w-[65px]' : ''}`}
                          style={columnWidth ? { width: columnWidth, minWidth: columnWidth } : undefined}
                        >
                      ...
                      {filteredAndSortedData.map((projection: SavesProjection, index) => {
                        ...
                        <tr key={projection.playerId} className={`border-b border-gray-100 hover:bg-blue-50/50 ${index < 10 ? 'bg-blue-50/30' : ''}`}>
                          <td className="py-2 px-1 md:px-3 sticky left-0 bg-white border-r border-gray-200 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.1)] z-20 w-[130px] min-w-[130px]">
                            <PlayerNameCell
                              name={viewMode === "future" ? ((playerIdToWebName && playerIdToWebName.get(projection.playerId)) || projection.playerName) : projection.playerName}
                          ...
                          <td
                            className={`px-1 md:px-3 py-2 md:py-4 text-center border-l border-gray-300 md:sticky md:right-[65px] z-[5] ${hasAnyAdjustment ? 'bg-purple-50' : getSavesColor(adjustedAverage)} ${!columnWidth ? 'w-[65px] min-w-[65px]' : ''}`}
                            style={columnWidth ? { width: columnWidth, minWidth: columnWidth } : undefined}
                          >
                            {(() => {
