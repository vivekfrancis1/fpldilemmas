...
export default function TransferRecommendations() {
  ...
  // Memoized: draftToFplPicks was previously called directly in the render body, producing a new
  // array reference every render whenever the draft-fallback branch was active. That broke
  // memoization for cascadedState/applyRecommendedTransfers/optimizedTeam (all depend on this
  // value), so the entire lineup — including the pitch view — recomputed and re-rendered on every
  // render tick, visible as constant flicker on this page for anyone on the draft fallback path.
  const effectiveTeamPicks = useMemo(() => {
    return (usingDraftFallback && cachedDraft && bootstrapData)
      ? draftToFplPicks(cachedDraft, bootstrapData)
      : (adjustedRecommendations?.authenticatedTeamPicks || teamData?.picks);
  }, [usingDraftFallback, cachedDraft, bootstrapData, adjustedRecommendations, teamData]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50/30 p-4">
      <div className="w-full space-y-4 sm:space-y-6">
        ...
        {/* Data-source notice: which of the two very different data sources is powering this
            view. Public lookups (any manager ID, including one auto-restored from localStorage
            with no live connection) only ever see the last LOCKED gameweek's squad via FPL's
            public picks endpoint — not pending transfers or a just-activated chip for the
            upcoming gameweek. Connected accounts get a live, real-time read instead. Users were
            seeing this gap first-hand (e.g. a remembered Manager ID showing wildcard status with
            no FPL account connected this session) without any indication of which case applied. */}
            ...
            <Alert className="bg-blue-50 border-blue-200">
              <AlertDescription className="text-sm text-blue-800">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    {user ? (
                      <>Public data — this is Manager {searchedId}'s GW {currentGW} squad (the last gameweek FPL makes public), not a live read. Connect your FPL account to get recommendations based on your real-time GW {upcomingGW} team.</>
                    ) : (
                      <>Public data — this is Manager {searchedId}'s GW {currentGW} squad (the last gameweek FPL makes public), not a live read. Login to FPL Dilemmas and connect your FPL account to get recommendations based on your real-time GW {upcomingGW} team.</>
                    )}
                  </div>
                </div>
              </AlertDescription>
        ...
        {isOwnTeam && shouldUseAuthenticatedEndpoint && (
