...
export default function TeamGoalsAgainstProjections() {
  ...
  const { data: historyData, isLoading: historyLoading } = useQuery<TeamGoalsAgainstHistory>({
    queryKey: ["/api/team-goals-against-history", historySeason],
    queryFn: async () => {
      const url = historySeason
        ? `/api/team-goals-against-history?season=${encodeURIComponent(historySeason)}`
        : `/api/team-goals-against-history`;
      const response = await fetch(url);
      if (!response.ok) throw new Error('Failed to fetch team goals against history');
                ...
                <CardContent className="pt-0 pb-6">
                  {viewMode === "past" && (
                    <div className="flex flex-wrap items-center gap-2 mt-2 mb-1">
                      ...
                      <button
                        onClick={() => { const last = historyData?.lastFinishedGW || 24; setStartGameweek("1"); setEndGameweek(String(last)); }}
                        className="text-xs px-2.5 py-0.5 rounded-full border border-purple-300 bg-purple-50 text-purple-700 hover:bg-purple-100 cursor-pointer font-medium"
                      >
                        All GWs
                      </button>
            ...
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                <button onClick={() => setShowOpponent(!showOpponent)}
                  className={`ml-auto inline-flex items-center gap-1 chip-toggle rounded-full border text-xs font-medium px-3 sm:px-3.5 py-1 leading-none cursor-pointer transition-colors ${showOpponent ? 'bg-purple-100 text-purple-700 border-purple-300' : 'bg-gray-100 text-gray-500 border-gray-300'}`}>
