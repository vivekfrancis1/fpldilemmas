...
import { Shield, DollarSign, Filter, BarChart3, Trophy, Info } from "lucide-react";
...
interface MarketTeamCSProjection {
  id: number;
  team: string;
  teamShort: string;
  teamName: string;
  ...
  totalProjectedCleanSheets: number;
  averageCleanSheetOdds: number;
...
export default function MarketTeamCSProjections() {
  ...
  const { data: projectionsData, isLoading: projectionsLoading } = useQuery<MarketTeamCSProjection[]>({
    queryKey: ["/api/team-cs-projections-market"],
  });
    ...
    return projectionsData
      ...
      .sort((a, b) => {
        ...
        switch (sortBy) {
          ...
          case "average":
            return b.averageCleanSheetOdds - a.averageCleanSheetOdds;
          case "team":
            return a.teamShort.localeCompare(b.teamShort);
          default:
            return b.totalProjectedCleanSheets - a.totalProjectedCleanSheets;
        }
      });
  ...
  const getCleanSheetColor = (odds: number) => {
    if (odds >= 60) return 'bg-green-50 text-green-800 font-semibold';
    if (odds >= 45) return 'bg-blue-50 text-blue-800 font-medium';
    if (odds >= 30) return 'bg-yellow-50 text-yellow-800';
    if (odds >= 20) return 'bg-orange-50 text-orange-800';
      ...
      <div className="fpl-page-header">
        <div className="fpl-page-header-content">
          <div className="fpl-page-title">
            ...
            <h1>Market-Based Team Clean Sheets</h1>
          </div>
          <p className="fpl-page-subtitle">
            Clean sheet probabilities derived from defensive betting markets and spread data - the most accurate predictions available
          </p>
        </div>
      </div>
      ...
      <div className="fpl-section-spacing">
        ...
        <Alert className="mb-6 bg-blue-50 border-blue-200">
          ...
          <AlertDescription className="text-blue-800">
            <strong>Market-Powered Accuracy:</strong> These projections use clean sheet betting odds that incorporate real money stakes, 
            defensive injuries, attacking opponent strength, and professional trader insights for maximum accuracy.
          </AlertDescription>
        </Alert>
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={sortBy} onValueChange={setSortBy}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="average">Avg CS %</SelectItem>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Period CS Total</p>
                  <p className="text-2xl font-bold text-gray-900">{totalCleanSheets.overallTotal.toFixed(0)}%</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Avg CS/Week</p>
                  <p className="text-2xl font-bold text-gray-900">{totalCleanSheets.averagePerGame.toFixed(0)}%</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{totalCleanSheets.seasonTotal.toFixed(0)}%</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-900">CS Markets</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="overflow-hidden">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Market-Based Clean Sheet Projections ({filteredProjections.length} teams)
            </CardTitle>
          </CardHeader>
                      ...
                      <tr key={team.id} className="hover:bg-gray-50" data-testid={`team-row-${team.teamShort}`}>
                        ...
                        <td className="px-4 py-4 text-center">
                          <span className="text-sm text-gray-900 font-medium">
                            {team.totalProjectedCleanSheets.toFixed(1)}
                          </span>
                        </td>
                        
                        <td className="px-4 py-4 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getCleanSheetColor(periodAverage)}`}>
                            {periodAverage.toFixed(0)}%
                          </span>
                        </td>
                        
                        {Array.from({ length: endGW - startGW + 1 }, (_, i) => {
                          const gw = startGW + i;
                          const odds = team.gameweekProjections[gw] || 0;
                          return (
                            <td key={gw} className="px-2 py-4 text-center">
                              <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${getCleanSheetColor(odds)}`}>
                                {odds.toFixed(0)}%
                              </span>
                            </td>
                          );
        ...
        <Card className="mt-6">
          <CardHeader>
            <CardTitle className="text-lg">About Market-Based Clean Sheet Projections</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-semibold text-gray-900 mb-2">Clean Sheet Market Data</h4>
                <ul className="text-sm text-gray-600 space-y-1">
                  <li>• Clean sheet betting odds from major bookmakers</li>
                  <li>• Defensive handicap lines and team totals</li>
                  <li>• Both teams to score (BTTS) market analysis</li>
                  <li>• Live odds movements reflecting team news</li>
                </ul>
              </div>
              <div>
                ...
                <ul className="text-sm text-gray-600 space-y-1">
                  ...
                  <li>• Instant incorporation of defensive injuries</li>
                  <li>• Opponent attacking strength analysis</li>
                  <li>• Weather and pitch condition factors</li>
                </ul>
              </div>
            </div>
          </CardContent>
